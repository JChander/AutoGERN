...
from src.utils_LP_hardsplit import *
import os
import sys
os.environ['NUMEXPR_MAX_THREADS'] = '64'

def main():
    parser = argparse.ArgumentParser('Interface for AutoGERN framework')
    ...
    parser.add_argument('--hidden_features', type=int, default=256, help='hidden dimension')
    parser.add_argument('--metric', type=str, default='auc', help='metric for evaluating performance', choices=['acc', 'auc'])
    parser.add_argument('--seed', type=int, default=42, help='seed to initialize all the random modules')
    ...
    parser.add_argument('--use_degree', type=bool, default=False, help='whether to use node degree as the initial feature')
    ...
    parser.add_argument('--retrain_epoch', type=int, default=20, help='number of epochs to retrain')
    ...
    model = search(model, dataloaders, args, logger)
    # save_performance_result(args, logger, results, mode='search')
    model, test_predictions, test_labels = retrain(model, dataloaders, args, logger)
    np.save(args.log_dir + args.dataset +'/test_predictions.npy', test_predictions)
    np.save(args.log_dir + args.dataset +'/test_labels.npy', test_labels)
