...
def read_file(args, logger):
    ...
    elif dataset in ['hESC500', 'hHEP500', 'mHSC-L500', 'mHSC-E500', 'mHSC-GM500', 'mESC500', 'mDC500', 'hESC1000', 'hHEP1000', 'mHSC-L1000', 'mHSC-E1000', 'mHSC-GM1000', 'mESC1000', 'mDC1000']:
...
def extract_subgaphs(G, labels, set_indices, prop_depth, layers, feature_flags, task, max_sprw, parallel, logger, debug=False):
    ...
    if not parallel:
        ...
        # Prepare graph data once; pass to workers via pool initializer to avoid per-task pickling
        edges = list(G.edges())
        ...
        # Use maxtasksperchild to avoid memory bloat and long-lived workers
        try:
            ctx = mp.get_context('fork')
        except ValueError:
            ctx = mp  # fallback (e.g., on non-Unix platforms)
        ...
        with ctx.Pool(processes=num_workers,
            ...
            # Use imap with chunksize for better progress tracking and memory efficiency
            # chunksize helps balance load and reduce communication overhead
            # Use small chunksize to reduce head-of-line blocking; 1 is safest for skewed workloads
            chunksize = 1
