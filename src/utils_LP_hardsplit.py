...
from models_intra import GNNModel
import random
import os
...
# Global variables for shared graph data in worker processes
_shared_graph_data = None
...
def read_edges(dir, node_id_mapping=None):
    edges = []
    # Try edges.csv first, fallback to network.csv
    if os.path.exists(dir + 'edges.csv'):
        fin_edges = pd.read_csv(dir + 'edges.csv', header=0)
    elif os.path.exists(dir + 'network.csv'):
        fin_edges = pd.read_csv(dir + 'network.csv', header=0)
    else:
        raise FileNotFoundError(f"Neither edges.csv nor network.csv found in {dir}")
    for index, row in fin_edges.iterrows():
        node1, node2 = row[0], row[1]
        if node_id_mapping is not None:
            node1 = node_id_mapping[node1]
            node2 = node_id_mapping[node2]
...
def read_file(args, logger):
    ...
    elif dataset in ['hESC500', 'hHEP500', 'mHSC-L500', 'mHSC-E500', 'mHSC-GM500', 'mESC500', 'mDC500', 'hESC1000', 'hHEP1000', 'mHSC-L1000', 'mHSC-E1000', 'mHSC-GM1000', 'mESC1000', 'mDC1000', 'hESC500_HardSplit', "hESC1000_HardSplit", 'hHEP500_HardSplit', 'mESC500_HardSplit', 'mESC1000_HardSplit', 'mHSC-L500_HardSplit', 'mHSC-E500_HardSplit', 'mHSC-GM500_HardSplit', 'mDC500_HardSplit', 'mDC1000_HardSplit', 'mHSC-GM1000_HardSplit', 'mHSC-L1000_HardSplit', 'mHSC-E1000_HardSplit', 'hHEP1000_HardSplit']:
    ...
    expr = pd.read_csv(directory + 'ExpressionData.csv', header = 0, index_col = 0)
    # Create node ID mapping from gene names to indices
    node_id_mapping = {gene_name: idx for idx, gene_name in enumerate(expr.index)}
    edges = read_edges(directory, node_id_mapping=node_id_mapping)
    nodelist = list(range(0,expr.shape[0]))
    if not di_flag:
        G = nx.Graph()
...
def get_data(G, task, args, labels, logger):
    ...
    if task == 'simulation':
        ...
        return loader
    G, labels_info, set_indices_info, split_meta = generate_samples_labels_graph(G, labels, task, args, logger)
    if args.debug:
        logger.info(list(G.edges))

    if split_meta.get('type') == 'predefined':
        datasets = {}
        for split_name in ['train', 'valid', 'test']:
            split_indices = set_indices_info.get(split_name)
            split_labels = labels_info.get(split_name)
            if split_indices is None or split_labels is None or split_indices.size == 0:
                datasets[split_name] = []
                continue
            datasets[split_name] = extract_subgaphs(
                G, split_labels, split_indices,
                prop_depth=args.prop_depth, layers=args.layers,
                feature_flags=feature_flags, task=task,
                max_sprw=(args.max_sp, args.rw_depth), parallel=args.parallel,
                logger=logger, debug=args.debug
            )
        train_set = datasets.get('train', [])
        val_set = datasets.get('valid', [])
        ...
        logger.info('使用预定义划分: 训练集 {} 条，验证集 {} 条，测试集 {} 条'.format(len(train_set), len(val_set), len(test_set)))
        all_label_arrays = [labels_info[name] for name in labels_info if labels_info[name] is not None and labels_info[name].size > 0]
        class_count = len(np.unique(np.concatenate(all_label_arrays))) if all_label_arrays else 0
        return (train_loader, val_loader, test_loader), class_count

    # 兼容历史逻辑
    labels = labels_info
    set_indices = set_indices_info
    train_mask, val_test_mask = split_meta['masks']
    data_list = extract_subgaphs(G, labels, set_indices, prop_depth=args.prop_depth, layers=args.layers,
                                 feature_flags=feature_flags, task=task,
                                 max_sprw=(args.max_sp, args.rw_depth), parallel=args.parallel, logger=logger, debug=args.debug)
...
def generate_samples_labels_graph(G, labels, task, args, logger):
    data_dir = os.path.join('./data', task, args.dataset)
    if not data_dir.endswith(os.sep):
        data_dir += os.sep
    split_files = ['Train_set.csv', 'Validation_set.csv', 'Test_set.csv']
    predefined_available = all(os.path.exists(os.path.join(data_dir, fname)) for fname in split_files)

    if labels is None and predefined_available:
        logger.info('检测到预定义的 train/valid/test 划分文件，直接加载样本集合。')
        raw_splits = load_predefined_pair_splits(data_dir, logger)
        raw_splits = apply_data_usage_to_train_split(raw_splits, args.data_usage, logger)
        set_indices_map = {name: pair_labels[0] for name, pair_labels in raw_splits.items()}
        labels_map = {name: pair_labels[1] for name, pair_labels in raw_splits.items()}
        G = G.to_undirected()
        removed_edges = remove_positive_edges_from_graph(G, set_indices_map, labels_map)
        if removed_edges:
            logger.info(f'为验证/测试集移除 {removed_edges} 条正样本边以避免信息泄露。')
        total_samples = sum(indices.shape[0] for indices in set_indices_map.values())
        all_label_arrays = [labels_map[name] for name in labels_map if labels_map[name] is not None and labels_map[name].size > 0]
        class_count = len(np.unique(np.concatenate(all_label_arrays))) if all_label_arrays else 0
        logger.info(f'预定义划分总样本数 {total_samples}，类别数 {class_count}。')
        split_meta = {'type': 'predefined', 'class_count': class_count}
        return G, labels_map, set_indices_map, split_meta

    if labels is None:
        logger.info('未检测到预定义划分，按原逻辑随机生成训练/验证/测试实例。')
        ...
        split_meta = {'type': 'random', 'masks': (train_mask, val_test_mask)}
        return G, labels, set_indices, split_meta
    ...
    logger.info('Generate {} train+val+test instances in total. data_usage: {}.'.format(set_indices.shape[0], args.data_usage))
    split_meta = {'type': 'node', 'masks': (train_mask, val_test_mask)}
    return G, labels, set_indices, split_meta
...
def load_predefined_pair_splits(directory, logger):
    required_columns = {'TF', 'Target', 'Label'}
    split_mapping = [
        ('train', 'Train_set.csv'),
        ('valid', 'Validation_set.csv'),
        ('test', 'Test_set.csv'),
    ]
    splits = {}
    for split_name, file_name in split_mapping:
        file_path = os.path.join(directory, file_name)
        if not os.path.exists(file_path):
            raise FileNotFoundError(f'预定义划分文件 {file_path} 不存在')
        df = pd.read_csv(file_path, header = 0, index_col = 0)
        if not required_columns.issubset(df.columns):
            raise ValueError(f'{file_path} 缺少必要列 {required_columns}')
        set_indices = df[['TF', 'Target']].to_numpy(dtype=np.int32)
        split_labels = df['Label'].astype(np.int32).to_numpy()
        splits[split_name] = (set_indices, split_labels)
        pos_count = int((split_labels == 1).sum())
        neg_count = int((split_labels == 0).sum())
        logger.info(f'加载 {split_name} 划分: 总计 {set_indices.shape[0]} 条样本 (正样本 {pos_count}, 负样本 {neg_count}) 来自 {file_path}')
    return splits


def apply_data_usage_to_train_split(splits, data_usage, logger):
    if 'train' not in splits or data_usage >= 1 - 1e-6:
        return splits
    train_indices, train_labels = splits['train']
    n_total = train_indices.shape[0]
    n_keep = int(round(n_total * data_usage))
    if n_keep <= 0:
        raise ValueError(f'data_usage={data_usage} 过小，导致训练样本数量为 0')
    selected = np.random.choice(n_total, n_keep, replace=False)
    splits['train'] = (train_indices[selected], train_labels[selected])
    logger.info(f'根据 data_usage={data_usage:.4f} 对训练集下采样: {n_total} -> {n_keep}')

    # sample the valid and test set
    valid_indices, valid_labels = splits['valid']
    test_indices, test_labels = splits['test']
    n_total_valid = valid_indices.shape[0]
    n_total_test = test_indices.shape[0]
    n_keep_valid = int(round(n_total_valid * 0.4))
    n_keep_test = int(round(n_total_test * 0.4)) 
    if n_keep_valid <= 0:
        raise ValueError(f'data_usage={data_usage} 过小，导致验证集样本数量为 0')
    if n_keep_test <= 0:
        raise ValueError(f'data_usage={data_usage} 过小，导致测试集样本数量为 0')
    selected_valid = np.random.choice(n_total_valid, n_keep_valid, replace=False)
    splits['valid'] = (valid_indices[selected_valid], valid_labels[selected_valid])
    selected_test = np.random.choice(n_total_test, n_keep_test, replace=False)
    splits['test'] = (test_indices[selected_test], test_labels[selected_test])
    logger.info(f'根据 data_usage={data_usage:.4f} 对验证集下采样: {n_total_valid} -> {n_keep_valid}')
    logger.info(f'根据 data_usage={data_usage:.4f} 对测试集下采样: {n_total_test} -> {n_keep_test}')

    return splits


def remove_positive_edges_from_graph(G, set_indices_map, labels_map, splits=('valid', 'test')):
    edges_to_remove = set()
    for split in splits:
        if split not in set_indices_map or split not in labels_map:
            continue
        split_indices = set_indices_map[split]
        split_labels = labels_map[split]
        if split_indices is None or split_labels is None:
            continue
        for sample, label in zip(split_indices, split_labels):
            if label != 1:
                continue
            sample_nodes = list(map(int, sample))
            for node_pair in combinations(sample_nodes, 2):
                edges_to_remove.add(tuple(node_pair))
    if edges_to_remove:
...
def extract_subgaphs(G, labels, set_indices, prop_depth, layers, feature_flags, task, max_sprw, parallel, logger, debug=False):
    ...
    if not parallel:
        # 非并行模式：逐个样本处理，优化内存和速度平衡
        # 减少gc.collect()频率以提升速度，但仍定期清理避免OOM
        gc_interval = 500  # 每500个样本才触发一次gc，减少开销
        for sample_i in tqdm(range(n_samples), desc='Processing samples'):
            ...
            # 只在特定间隔触发gc，减少性能开销
            if (sample_i + 1) % gc_interval == 0:
                gc.collect()
    else:
        # 通过初始化函数共享图结构，避免反复序列化导致内存激增
        edges = list(G.edges())
        attributes = G.graph.get('attributes', None)
        if attributes is not None:
            attributes = np.array(attributes, copy=False)
        is_directed = isinstance(G, nx.DiGraph)
        num_nodes = G.number_of_nodes()
        
        # Prepare lightweight arguments per sample (avoid sending edges/attributes repeatedly)
        args_list = []
        for sample_i in range(n_samples):
            args_list.append((
                set_indices[sample_i],
                hop_num,
                feature_flags,
                max_sprw,
                labels[sample_i] if labels is not None else None,
                debug
            ))
        
        # Use a reasonable number of workers (not too many to avoid memory issues)
        num_workers = min(16, mp.cpu_count(), n_samples)
        ...
        with ctx.Pool(processes=num_workers,
                      ...
                      initializer=_init_parallel_graph,
                      initargs=(edges, attributes, is_directed, num_nodes)) as pool:
            chunksize = 1
            iterator = pool.imap_unordered(parallel_worker_shared, args_list, chunksize=chunksize)
            with tqdm(total=n_samples, desc='Processing samples') as pbar:
                ...
                try:
                    while processed < n_samples:
                        try:
                            result = iterator.next(timeout=600)
                        except TimeoutError:
                            logger.error('Timeout while waiting for worker results (>=600s). '
                                         'Likely a worker process crashed or is hung.')
                            raise
                        except Exception:
                            raise
                        data_list.append(_unpack_data_result(result))
                        processed += 1
                        pbar.update(1)
...
def parallel_worker_improved(args):
    """Legacy improved worker kept for compatibility (not used by default)."""
    edges, attributes, is_directed, num_nodes, set_index, hop_num, feature_flags, max_sprw, label, debug = args
    return _build_and_run(edges, attributes, is_directed, num_nodes, set_index, hop_num, feature_flags, max_sprw, label, debug)


def _init_parallel_graph(edges, attributes, is_directed, num_nodes):
    """Initializer for worker processes to receive shared graph data."""
    global _shared_graph_data
    _shared_graph_data = (edges, attributes, is_directed, num_nodes)


def parallel_worker_shared(args):
    """Worker that reconstructs the graph from globally shared data to avoid large queue payloads."""
    global _shared_graph_data
    edges, attributes, is_directed, num_nodes = _shared_graph_data
    set_index, hop_num, feature_flags, max_sprw, label, debug = args
    return _build_and_run(edges, attributes, is_directed, num_nodes, set_index, hop_num, feature_flags, max_sprw, label, debug)


def _build_and_run(edges, attributes, is_directed, num_nodes, set_index, hop_num, feature_flags, max_sprw, label, debug):
    # Normalize set_index type
    if isinstance(set_index, np.ndarray):
        set_index = set_index.tolist()
    elif not isinstance(set_index, list):
    ...
    if attributes is not None:
        G.graph['attributes'] = attributes
    data = get_data_sample(G, set_index, hop_num, feature_flags, max_sprw, label, debug=debug)
    return _pack_data_result(data, debug)


def _pack_data_result(data, debug):
    """Convert torch_geometric.data.Data into lightweight numpy-based dict for transport."""
    result = {
        'x': data.x.cpu().numpy(),
        'edge_index': data.edge_index.cpu().numpy(),
        'y': data.y.cpu().numpy(),
        'set_indices': data.set_indices.cpu().numpy(),
        'debug': debug,
    }
    if debug:
        if hasattr(data, 'old_set_indices') and data.old_set_indices is not None:
            result['old_set_indices'] = data.old_set_indices.cpu().numpy()
        if hasattr(data, 'old_subgraph_indices') and data.old_subgraph_indices is not None:
            result['old_subgraph_indices'] = data.old_subgraph_indices.cpu().numpy()
    return result


def _unpack_data_result(result):
    """Reconstruct torch_geometric.data.Data from numpy-based dict."""
    data = Data(
        x=torch.from_numpy(result['x']),
        edge_index=torch.from_numpy(result['edge_index']),
        y=torch.from_numpy(result['y']),
        set_indices=torch.from_numpy(result['set_indices'])
    )
    if result.get('debug', False):
        if 'old_set_indices' in result:
            data.old_set_indices = torch.from_numpy(result['old_set_indices'])
        if 'old_subgraph_indices' in result:
            data.old_subgraph_indices = torch.from_numpy(result['old_subgraph_indices'])
    return data


def parallel_worker(x):
    """Legacy worker function - kept for backward compatibility"""
...
def get_data_sample(G, set_index, hop_num, feature_flags, max_sprw, label, debug=False):
    ...
    if len(set_index) > 1:
        G_work = G.copy()
        G_work.remove_edges_from(combinations(set_index, 2))

    edge_index = torch.tensor(list(G_work.edges)).long().t().contiguous()
    edge_index = torch.cat([edge_index, edge_index[[1, 0], ]], dim=-1)
    subgraph_node_old_index, new_edge_index, new_set_index, edge_mask = tgu.k_hop_subgraph(
        torch.tensor(set_index).long(), hop_num, edge_index, num_nodes=G_work.number_of_nodes(), relabel_nodes=True)

    # 删除临时图副本（如果创建了），但不立即gc，让Python自动回收
    if G_work is not G:
        del G_work
    ...
    if sp_flag:
        ...
        # 小变量让Python自动回收，不手动删除

    if rw_flag:
        # 优化：对于大子图，先检查是否需要稠密矩阵
        # 如果子图很大，直接使用稀疏矩阵计算（如果可能）或延迟转换
        adj_sparse = nx.adjacency_matrix(new_G, nodelist=np.arange(new_G.number_of_nodes(), dtype=np.int32))
        adj_sparse = adj_sparse.astype(np.float32)
        
        # 提高阈值，减少不必要的稠密转换
        # 对于中等大小的图，直接转换；对于很大的图，转换后立即删除稀疏版本
        if num_nodes < 1000:  # 提高阈值，减少转换开销
            adj = np.asarray(adj_sparse.todense())
        else:
            # 大图：转换后立即删除稀疏版本以释放内存
            adj = adj_sparse.toarray()
            del adj_sparse  # 立即删除稀疏版本
        ...
        # 只删除大的邻接矩阵，其他让Python自动回收
        del adj

    # 删除子图对象（可能较大），但不在函数内gc
    del new_G
...
def get_features_sp_sample(G, node_set, max_sp):
    ...
    for i, node in enumerate(node_set):
        # Use single_source_shortest_path_length to support environments without 'cutoff' keyword
        for node_ngh, length in nx.single_source_shortest_path_length(G, node, max_sp).items():
...
def split_datalist(data_list, masks):
    ...
    val_test_labels = np.array([data.y.item() for data in data_list], dtype=np.int32)[val_test_indices]
