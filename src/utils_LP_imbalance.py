...
def read_edges(dir):
    edges = []
    fin_edges = pd.read_csv(dir + 'edges.csv', header=0, index_col=0)
...
def read_file(args, logger):
    ...
    elif dataset in ['arxiv', 'celegans', 'celegans_small', 'facebook', 'hESC500', 'hHEP500', 'mHSC-L500', 'mHSC-E500', 'mHSC-GM500', 'yeast', 'mESC500', 'mDC500']:
...
def generate_set_indices_labels(G, task, test_ratio, data_usage=1.0):
    ...
    test_mask = get_mask(test_pos_indices + test_neg_indices, length=n_pos_edges + n_neg_edges)
    train_mask = np.ones_like(test_mask) - test_mask
    labels = np.concatenate([np.ones((n_pos_edges, )), np.zeros((n_neg_edges, ))]).astype(np.int32)
    ...
    permutation = np.random.permutation(n_pos_edges + n_neg_edges)
...
def sample_pos_neg_sets(G, task, data_usage=1.0):
    ...
    neg_edges = np.array(sample_neg_sets(G, pos_edges.shape[0] * 400, set_size=set_size), dtype=np.int32)
